...
import { Wrench, Utensils, Calendar } from 'lucide-react';

const EquipmentAndMenu = () => {
  const [subTab, setSubTab] = useState('equipment'); // 'equipment' | 'menu' | 'maintenance'
  const { state } = useApp();
    ...
    <div className="space-y-4">
      <div className="flex justify-end">
        <div className="inline-flex p-1 bg-gray-100 rounded-lg">
          ...
          <button type="button" onClick={() => setSubTab('maintenance')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${subTab === 'maintenance' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>
      ...
      {subTab === 'maintenance' && <MaintenanceScheduleView equipmentData={state.equipmentData} />}
